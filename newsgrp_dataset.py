...
class NewsGroup_DataSet:
    ...
    def __init__(self, args):
        ...
        self.data_dir = args.data_path + '20news/'
    ...
    def generate_data(self, val_ratio=.1, shuffle=True):
        print("generating 20 newsgroup data ...")
    ...
    def load_data_if_not_exist(self):
        ...
        train_df = pd.DataFrame(
            {'Text': [self.sent_parse(text) for text in newsgroups_train.data],
             'Class': newsgroups_train.target}
        )

        test_df = pd.DataFrame(
            {'Text': [self.sent_parse(text) for text in newsgroups_test.data],
             'Class': newsgroups_test.target}
        )
